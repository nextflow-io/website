...
for the cloud, implementing a native support for Amazon EFS and an optimised cloud native
...
When the cluster is ready, the only requirement is to SSH login to the cluster master 
node and launch the Nextflow pipeline execution as usual. 
...
* For the sake of this demo we are using [paraMSA](https://github.com/pditommaso/paraMSA), 
  ...
  in our lab by [Evan Floden](https://github.com/skptic). 
...
We are releasing the Nextflow integrated cloud support as a technological preview.  
You can use it by installing the Nextflow development snapshot as shown below: 
