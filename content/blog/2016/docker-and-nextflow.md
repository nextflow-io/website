...
Recently I have become involved with a collaboration between several groups across Europe with the aim to annotate long non-coding RNA (lncRNA) in farm animals. I have joined with the annoation based on RNA sequencing data. The basic procceedure relies on mapping short read data from many tissues, generating transcripts and then determining if they are likely to be lncRNA or protein coding genes.

During several successful 'hackathon' meetings the best approach was determined and implemented in a joint effort. I undertook the task to wrap the whole proceedure up into a Nextflow pipeline, the aim being so we can replicate results across our different institutions and allow the easy application of the pipeline by researchers anyway. 

Creating the Nextflow pipleine ([here](http://www.github.com/skptic/lncrna-annotation-nf)) in itself was not a difficult task. My collabrators had documented their work well and were on hand if anything was not clear. Problems however arose when we tried to port and replicate the results across the different institutions. 

The pipeline is typical of many in bioinformatics, consisting of binary executions, bash scripting, R, perl, bioperl and some custom perl modules. We found the bio-perl modules in particular where very sensistive to the various versions in the *long* depenancy tree. The solution was to turn to [Docker](link to docker and Nextflow blog). 

I have taken this oportunity to document the process of developing the Docker side of a Nextflow + Docker pipeline step-by-step, both for myself and with the hope it may encourage others, who may be a little wary, to give it a go and join in the virtulisation revolution!

###Docker Installation
By far the most challenging issue is the installation of Docker. For local installations, the [process is relativly straight forward](https://docs.docker.com/engine/installation). However difficulties arise as computing moves to the cluster. Owing to security concerns, many HPC administors have been reluctant to install Docker system-wide. This is changing and Docker developers have been responding to many of these concerns with updates addressing these issues.
...
This sets the base distribution for our Docker image to be debian v8.4, a lightweight linux distrubtion that is ideally suited to the task. We must also specify the maintainer of Docker image.
...
We find it very helpful to test our images as we develop the Docker file. Once built, it is possible to launch the Docker image and test if the desired software was correctly installed. For example, we can test if FEELnc and its depandancies were succesfully installed by running the following:
...
Once you are confident your image is built correctly, you can tag it, allowing you to push it to [Dockerhub.io](http://www.dockerhub.io). Dockerhub is an online reposityory for docker images which allows anyone to pull public images and run them. To find 
...
If you have not previously, sign up for a Dockerhub account [here](http://www.dockerhub sign-up). From the command line, login to Dockerhub and push your image.

    docker login --username=skptic
    docker push skptic/lncrna_annotation
    
You can test if you image has been correctly pushed and is publically availble by removing your local version using the IMAGE ID of the image and pulling the remote:
...
In most cases it is easiest to use the same docker image for all processes. One futher thing to consider is the inclusion of the sha256 hash of the image in the container reference. I have [previously written about this](http://www.nextflow.io/blog/2016/best-practice-for-reproducibility.html), but briefly, including a hash ensures that not a single byte of the operating system or software is different.
...
Within a GitHub README.md you can then add a badge with the following: 

    ![CircleCI status](https://circleci.com/gh/skptic/lncRNA-Annotation-nf.png?style=shield)

###Tips and Tricks
**File permissions**: When a process is executed by a docker container, the unix user running the process is not you. Therefore any files that are used as an input should have the appropriate file permissions. For example, I had to change all the input data in the test data set with:

    chmod 0755 tutorial/*/*

**Scratch disk**: Related to this, Docker must be able to write files. **@PAOLO**: This is why we need `scratch=true`? should I mention this? ie is this normal for most HPC users of Docker?