...
difficulties related on keeping keeping such a piece of software consistent. 

Computing environments are updated frequently in order to test new pieces of software or 
because a new versions are required. For this reason replicating data analysis results 
over time can be a challenging most of the times. 
...
For this reason in our lab we decided to test the capabilities of Docker in the cluster provided by our [institute](http://www.crg.eu).

## Method

The Docker engine has been installed in each node in our cluster, that runs a [Univa grid engine](http://www.univa.com/products/grid-engine.php) resource manager. 
...
tries to run the job to a node where that image has already been already pulled, 
...
it with and without Docker and comparing the resulting execution times.

The pipeline execution launches around 100 jobs, 10 cluster nodes were used. It was run 5 times 
...
For this reason the actual task run time was measured as well i.e. without including the Docker bootstrap 
time overhead. In this case, the aggregate average task execution time was 57.3 minutes
...
to replicate the results without the usual burden required to set-up and identical computing environment.    
